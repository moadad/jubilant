...
function renderProducts(items) {
  ...
  items.forEach((product) => {
    ...
    card.querySelector('.product-sub').innerHTML = `<span>موديل ${escapeHTML(product.model || '-')}</span><span>${escapeHTML(product.sizes || '')}</span>`;
...
function renderCart() {
  ...
  el.cartItemsCount.textContent = `${count} قطعة`;
  ...
  products.forEach((product) => {
    ...
    item.innerHTML = `
      ...
      <div>
        ...
        <div class="muted">${hasDiscount(product) ? `خصم ${Math.round(toNumber(product.discountPercent))}% • ` : ''}${formatCurrency(getDisplayPrice(product))}</div>
      </div>
...
function renderAdminOrders() {
  ...
  state.orders.forEach((order) => {
    ...
    card.innerHTML = `
      ...
      <div class="muted" style="margin-top:8px">${(order.items || []).map((item) => `${item.name} (${item.qty})`).join(' • ')}</div>
...
function populateProductForm(product) {
  ...
  el.productMinQtyInput.value = String(Math.max(1, toInt(product.minQty || 1)));
...
async function addToCart(productId) {
  ...
  else state.cart.push({ id: product.id, name: product.name, model: product.model, unitPrice, originalPrice: toNumber(product.priceWholesale), discountPercent: toNumber(product.discountPercent || 0), imageUrl: normalizeImageUrls(product.imageUrls)[0] || '', qty: minQty });
  renderCart();
  showToast('تمت إضافة المنتج');
}
...
async function submitOrder() {
  ...
  const order = {
    ...
    items: state.cart.map((item) => ({ productId: item.id, name: item.name, model: item.model, unitPrice: item.unitPrice, qty: item.qty, imageUrl: item.imageUrl })),
  ...
  try {
    ...
    rows.forEach((row) => {
      ...
      batch.set(ref, {
        ...
        minQty: Math.max(1, toInt(firstValue(row, ['minQty', 'الحد الأدنى']) || 1)),
...
function buildWhatsAppOrderMessage(order) {
  return [
    ...
    'المنتجات:',
    ...(order.items || []).map((item) => `- ${item.name || ''} | موديل ${item.model || ''} | الكمية ${item.qty || 0} | ${formatCurrency(item.unitPrice || 0)}`),
