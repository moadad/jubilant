...
const state = {
  ...
  cart: loadLocalJSON('joodkids_cart_wholesale_piece_v2', []),
  authUser: null,
  isAdmin: false,
...
function bindUI() {
  ...
  el.adminDrawer.addEventListener('click', (event) => { event.stopPropagation(); });
...
function renderStorefront() {
  ...
  setActionLink(el.instagramLink, state.company.instagram);
  setActionLink(el.facebookLink, state.company.facebook);
  setActionLink(el.telegramLink, state.company.telegram);
  setActionLink(el.whatsappLink, buildWhatsAppLink());
...
function renderCart() {
  saveLocalJSON('joodkids_cart_wholesale_piece_v2', state.cart);
