...
const APP_SW_VERSION = 'joodkids-fast-secure-v12';
...
const state = {
  ...
  gallery: { urls: [], index: 0, product: null, touchStartX: 0, touchStartY: 0 },
...
const el = {
  ...
  galleryMetaCard: id('galleryMetaCard'),
  galleryMetaTitle: id('galleryMetaTitle'),
  galleryMetaBadge: id('galleryMetaBadge'),
  galleryMetaChips: id('galleryMetaChips'),
  galleryMetaPrice: id('galleryMetaPrice'),
  galleryMetaDesc: id('galleryMetaDesc'),
...
function bindUI() {
  ...
  bindPress(el.closeMenu, closeDrawers, { prevent: true, stop: true });
  bindPress(el.brandTrigger, () => openDrawer('admin'));
  bindPress(el.cartToggle, () => openDrawer('cart'));
  bindPress(el.closeCart, closeDrawers, { prevent: true, stop: true });
  bindPress(el.closeAdmin, closeDrawers, { prevent: true, stop: true });
  ...
  bindPress(el.overlay, closeDrawers, { prevent: true, stop: true });
  bindPress(el.contactBtn, () => openModal('contactModal'));
  bindPress(el.menuContactBtn, () => { closeDrawers(); openModal('contactModal'); });
  bindPress(el.menuNoticeBtn, () => openPolicy('التنويه', state.storeSettings.shippingPolicy || DEFAULT_SHIPPING_POLICY));
  bindPress(el.menuPaymentBtn, () => openPolicy('سياسة الدفع', state.payments.policyText || DEFAULT_PAYMENT_POLICY));
  bindPress(el.menuReturnBtn, () => openPolicy('سياسة الاستبدال والاسترجاع', state.storeSettings.returnPolicy || DEFAULT_RETURN_POLICY));
  bindPress(el.menuTermsBtn, () => openPolicy('الشروط', state.storeSettings.termsPolicy || DEFAULT_TERMS_POLICY));
  bindPress(el.menuOffersBtn, () => {
  ...
  bindPress(el.installBtn, installPwa);
  bindPress(el.menuInstallBtn, installPwa);
  ...
  bindPress(el.clearFiltersBtn, clearFilters);
  bindPress(el.loadMoreBtn, renderMoreProducts);
  bindPress(el.floatingWhatsApp, openWhatsAppDirect);
  ...
  bindPress(el.checkoutBtn, () => {
    if (!state.cart.length) return showToast('السلة فارغة');
    openModal('checkoutModal');
  });
  bindPress(el.submitOrderBtn, submitOrder);
  bindPress(el.adminLoginBtn, adminLogin);
  bindPress(el.adminLogoutBtn, adminLogout);
  bindPress(el.saveAppearanceBtn, saveAppearance);
  bindPress(el.saveCompanyBtn, saveCompanyData);
  bindPress(el.savePoliciesBtn, savePolicies);
  bindPress(el.saveSeasonsBtn, saveSeasons);
  bindPress(el.saveProductBtn, saveProduct);
  bindPress(el.resetProductBtn, resetProductForm);
  ...
  bindPress(el.exportProductsBtn, exportProductsExcel);
  bindPress(el.exportOrdersBtn, exportOrdersExcel);
  el.excelImportInput.addEventListener('change', importProductsExcel);
  bindPress(el.deleteProductsBtn, () => deleteCollectionDocs('products', 'اكتب حذف المنتجات'));
  bindPress(el.deleteOrdersBtn, () => deleteCollectionDocs('orders', 'اكتب حذف الطلبات'));
  bindPress(el.deleteAllDataBtn, deleteAllData);
  bindPress(el.togglePinnedFilterBtn, togglePinnedAdminFilter);
  bindPress(el.galleryPrev, () => changeGallery(-1), { prevent: true, stop: true });
  bindPress(el.galleryNext, () => changeGallery(1), { prevent: true, stop: true });
  el.singleAssetUploader.addEventListener('change', handleSingleAssetUpload);
  el.uploadTriggers.forEach((btn) => bindPress(btn, () => {
    if (!guardAdmin()) return;
    state.assetTargetInputId = btn.dataset.targetInput;
    el.singleAssetUploader.click();
  }));
  document.querySelectorAll('.modal-close').forEach((btn) => bindPress(btn, (e) => closeModal(e.currentTarget.dataset.close), { prevent: true, stop: true }));
...
function initTabs() {
  el.adminTabs.forEach((btn) => bindPress(btn, (event) => {
...
function renderProducts() {
  ...
  renderedItems.forEach((product, index) => {
    ...
    const open = () => openGallery(urls.length ? urls : [placeholderImage(product.name || product.model || 'Jood Kids')], 0, product);
    bindPress(card.querySelector('.product-media'), open, { prevent: true, stop: true });
    bindPress(card.querySelector('.zoom-btn'), open, { prevent: true, stop: true });
    bindPress(card.querySelector('.add-btn'), () => addToCart(product.id));
...
function renderCart() {
  ...
  state.cart.forEach((item) => {
    ...
    bindPress(card.querySelector('[data-action="plus"]'), () => changeCartQty(item.id, 1));
    bindPress(card.querySelector('[data-action="minus"]'), () => changeCartQty(item.id, -1));
    bindPress(card.querySelector('.cart-thumb'), () => openGallery(item.imageUrl ? [item.imageUrl] : [placeholderImage(item.name || 'Jood Kids')], 0, item), { prevent: true, stop: true });
...
function renderCategoryManager() {
  ...
  keys.forEach((key) => {
    ...
    bindPress(wrapper.querySelector('button'), () => saveCodeCategoryLabel(key, wrapper.querySelector('input').value));
...
function renderAdminProducts() {
  ...
  products.forEach((product) => {
    ...
    bindPress(item.querySelector('[data-edit]'), () => populateProductForm(product));
    bindPress(item.querySelector('[data-pin]'), () => togglePinned(product));
    bindPress(item.querySelector('[data-del]'), () => deleteProduct(product.id));
...
function renderAdminOrders() {
  ...
  if (!state.isAdmin) {
    el.adminOrdersList.innerHTML = '<div class="order-item"><div class="muted">الطلبات تظهر فقط بعد دخول الأدمن الصحيح</div></div>';
    return;
  }
  ...
  state.orders.forEach((order) => {
    ...
    bindPress(card.querySelector('[data-copy]'), () => copyText(buildWhatsAppOrderMessage(order)));
    bindPress(card.querySelector('[data-del]'), () => deleteOrder(order.id));
...
function openGallery(urls, index = 0, product = null) {
  state.gallery.urls = normalizeImageUrls(urls);
  state.gallery.index = clamp(index, 0, Math.max(0, state.gallery.urls.length - 1));
  state.gallery.product = product ? { ...product } : null;
  syncGallery();
...
function syncGallery() {
  ...
  syncGalleryMeta();
  el.galleryPrev.classList.toggle('hidden', state.gallery.urls.length <= 1);
  el.galleryNext.classList.toggle('hidden', state.gallery.urls.length <= 1);
  el.galleryThumbs.innerHTML = '';
  state.gallery.urls.forEach((imageUrl, index) => {
    ...
    bindPress(button, () => { state.gallery.index = index; syncGallery(); }, { prevent: true, stop: true });
...
function bindPress(target, handler, { prevent = false, stop = false } = {}) {
...
function handleGlobalKeydown(event) {
  if (event.key === 'Escape') {
    const openModalElement = [...document.querySelectorAll('.modal.show')].at(-1);
    if (openModalElement) {
  ...
  if (!el.imageModal.classList.contains('show')) return;
...
function bindGalleryTouch() {
  ...
  el.imageModal.addEventListener('touchstart', (event) => {
    if (!el.imageModal.classList.contains('show')) return;
  ...
  el.imageModal.addEventListener('touchend', (event) => {
    if (!el.imageModal.classList.contains('show')) return;
    const touch = event.changedTouches?.[0];
    if (!touch) return;
    const deltaX = touch.clientX - (state.gallery.touchStartX || 0);
    const deltaY = touch.clientY - (state.gallery.touchStartY || 0);
...
function syncGalleryMeta() {
  const product = state.gallery.product;
  if (!product) {
    el.galleryMetaCard.classList.add('hidden');
    el.galleryMetaTitle.textContent = 'المنتج';
    el.galleryMetaBadge.classList.add('hidden');
    el.galleryMetaChips.innerHTML = '';
    el.galleryMetaPrice.innerHTML = '';
    el.galleryMetaDesc.textContent = '';
    return;
  }
  el.galleryMetaCard.classList.remove('hidden');
  el.galleryMetaTitle.textContent = product.name || `موديل ${product.model || '-'}`;
  const offer = hasDiscount(product);
  el.galleryMetaBadge.textContent = offer ? `خصم ${Math.round(toNumber(product.discountPercent))}%` : 'جملة';
  el.galleryMetaBadge.classList.toggle('hidden', !el.galleryMetaBadge.textContent);
  el.galleryMetaChips.innerHTML = '';
  [product.model ? `موديل ${product.model}` : '', getSeriesLabel(product), product.sizes || '', product.season || '', getCodeCategoryLabel(product.codeCategory)].filter(Boolean).forEach((text) => {
    el.galleryMetaChips.appendChild(makeBadge(text));
  });
  el.galleryMetaPrice.innerHTML = `
    ...
    <div class="gallery-price-note">سعر القطعة ${formatCurrency(getPiecePrice(product))}</div>`;
  el.galleryMetaDesc.textContent = product.description || '';
}
