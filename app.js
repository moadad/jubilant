...
const APP_SW_VERSION = 'joodkids-fast-secure-v16';
...
const state = {
  ...
  catalog: { step: 'seasons', selectedSeason: '', selectedCategory: 'all', hydratedFromUrl: false },
};
...
function bindUI() {
  ...
  [el.contactModal, el.policyModal, el.imageModal, el.checkoutModal, el.quickProductModal].filter(Boolean).forEach((modal) => {
    ['click', 'pointerup', 'touchend'].forEach((eventName) => {
...
function activateCatalogProducts(season = '', category = 'all', { closeAfter = false, scrollIntoView = true, preserveSort = false } = {}) {
  const safeSeason = String(season || '').trim();
...
function syncCatalogHero() {
  ...
  if (step === 'seasons') {
    ...
    title = categoryLabel || selectedSeason || defaultTitle;
    subtitle = selectedSeason ? (categoryLabel ? `${selectedSeason} • ${categoryLabel}` : `كل موديلات ${selectedSeason} مع تصنيف البادئة`) : defaultSubtitle;
    badge = categoryLabel ? `بادئة ${selectedCategory}` : (selectedSeason ? 'كل منتجات الموسم' : defaultBadge);
    sectionTitle = categoryLabel ? categoryLabel : (selectedSeason ? `موديلات ${selectedSeason}` : 'المنتجات');
  }
...
function renderCatalogFlow() {
  ...
  if (step === 'seasons') {
    ...
    const chips = prefixFilters.map((code) => {
      ...
      return `<button class="prefix-filter-chip ${selectedCategory === code ? 'active' : ''}" data-action="filter-prefix" data-season="${escapeAttr(selectedSeason)}" data-category="${escapeAttr(code)}"><span>${escapeHTML(label)}</span><b>${count.toLocaleString('en-US')}</b></button>`;
    }).join('');
    const title = selectedCategory === 'all' ? `كل موديلات ${selectedSeason || 'المتجر'}` : getCodeCategoryLabel(selectedCategory);
    const description = selectedCategory === 'all'
      ? 'جميع منتجات الموسم مع إمكانية التصفية حسب البادئة.'
      : `عرض موديلات ${getCodeCategoryLabel(selectedCategory)} داخل ${selectedSeason}.`;
    html = `
      <div class="catalog-browser-card">
        <div class="catalog-breadcrumb">
          <button class="crumb-btn" data-action="go-seasons">المواسم</button>
        ...
        <div class="prefix-filters-bar">${chips}</div>
      </div>`;
  }
  ...
  el.catalogFlow.querySelectorAll('[data-action="filter-prefix"]').forEach((btn) => btn.addEventListener('click', () => activateCatalogProducts(btn.dataset.season, btn.dataset.category, { preserveSort: true })));
...
function renderProducts() {
  ...
  renderedItems.forEach((product, index) => {
    ...
    card.querySelector('.product-model-line').textContent = `موديل ${product.model || '-'}`;
...
function getProductPageUrl(product) {
  ...
  if (model) return `${SITE_URL}product.html?model=${encodeURIComponent(model)}`;
...
function buildProductDescription(product) {
  ...
  const parts = [
    ...
    getCodeCategoryLabel(product?.codeCategory) ? `التصنيف: ${getCodeCategoryLabel(product.codeCategory)}` : '',
    `المتجر: ${brand}`
  ].filter(Boolean);
...
function openQuickProduct(product) {
  ...
  el.quickProductCategory.textContent = categoryLabel;
...
function clearFilters() {
  ...
  state.filter = { search: '', category: category || 'all', season: season || 'all', offersOnly: false, sort: 'featured' };
  el.searchInput.value = '';
  el.sortFilter.value = 'featured';
  resetRenderedProducts();
  applyFilters();
