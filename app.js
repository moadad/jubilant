...
const APP_SW_VERSION = 'joodkids-fast-secure-v11';
...
const state = {
  ...
  gallery: { urls: [], index: 0, product: null },
...
const el = {
  ...
  galleryInfoCard: id('galleryInfoCard'),
  galleryInfoBadge: id('galleryInfoBadge'),
  galleryInfoTitle: id('galleryInfoTitle'),
  galleryInfoMeta: id('galleryInfoMeta'),
  galleryInfoPrice: id('galleryInfoPrice'),
  galleryInfoPiece: id('galleryInfoPiece'),
  galleryInfoBadges: id('galleryInfoBadges'),
  galleryInfoDesc: id('galleryInfoDesc'),
...
function renderProducts() {
  ...
  renderedItems.forEach((product, index) => {
    ...
    const thumbSrcSet = buildCloudinarySrcSet(originalUrl, [280, 420, 560, 760]);
    card.innerHTML = `
      <div class="product-media" role="button" tabindex="0" aria-label="عرض ${escapeAttr(product.name || 'المنتج')}">
        <img src="${escapeAttr(thumbUrl)}" ${thumbSrcSet ? `srcset="${escapeAttr(thumbSrcSet)}" sizes="(max-width: 420px) 50vw, (max-width: 860px) 42vw, 25vw"` : ''} alt="${escapeAttr(product.name || 'product')}" loading="${index < 4 ? 'eager' : 'lazy'}" decoding="async" fetchpriority="${index < 4 ? 'high' : 'low'}" />
    ...
    const mediaButton = card.querySelector('.product-media');
    mediaButton.addEventListener('click', open);
    mediaButton.addEventListener('keydown', (event) => {
...
function renderCart() {
  ...
  state.cart.forEach((item) => {
    ...
    card.querySelector('.cart-thumb').addEventListener('click', () => openGallery(item.imageUrl ? [item.imageUrl] : [placeholderImage(item.name || 'Jood Kids')], 0, item));
...
function renderAdminOrders() {
  ...
  if (!state.isAdmin) {
    el.adminOrdersList.innerHTML = '<div class="order-item"><div class="muted">لا يمكن عرض الطلبات إلا بعد دخول الأدمن الصحيح</div></div>';
    return;
  }
...
function closeModal(modalId) {
  ...
  if (modalId === 'imageModal') resetGalleryMeta();
...
function renderGalleryMeta() {
  ...
  if (!product) return resetGalleryMeta();
  const badges = [getCodeCategoryLabel(product.codeCategory), product.season, product.sizes, getSeriesLabel(product), hasDiscount(product) ? 'عرض' : '', product.badgeText].filter(Boolean);
  el.galleryInfoCard.classList.remove('hidden');
  const badgeText = hasDiscount(product) ? `خصم ${Math.round(toNumber(product.discountPercent || 0))}%` : '';
  el.galleryInfoBadge.textContent = badgeText;
  el.galleryInfoBadge.classList.toggle('hidden', !badgeText);
  el.galleryInfoTitle.textContent = product.name || 'المنتج';
  el.galleryInfoMeta.innerHTML = [
    `موديل ${escapeHTML(product.model || '-')}`,
    escapeHTML(product.sizes || ''),
    escapeHTML(getSeriesLabel(product)),
  ].filter(Boolean).map((item) => `<span>${item}</span>`).join('');
  el.galleryInfoPrice.textContent = formatCurrency(getDisplayPrice(product));
  el.galleryInfoPiece.textContent = `سعر القطعة ${formatCurrency(getPiecePrice(product))}`;
  el.galleryInfoBadges.innerHTML = '';
  badges.forEach((text) => el.galleryInfoBadges.appendChild(makeBadge(text)));
  el.galleryInfoDesc.textContent = product.description || '';
}

function resetGalleryMeta() {
  state.gallery.product = null;
  el.galleryInfoCard.classList.add('hidden');
  el.galleryInfoBadge.textContent = '';
  el.galleryInfoBadge.classList.add('hidden');
  el.galleryInfoTitle.textContent = 'المنتج';
  el.galleryInfoMeta.innerHTML = '';
  el.galleryInfoPrice.textContent = formatCurrency(0);
  el.galleryInfoPiece.textContent = '';
  el.galleryInfoBadges.innerHTML = '';
  el.galleryInfoDesc.textContent = '';
}

function preloadGalleryNeighbors() {
  if (!state.gallery.urls.length) return;
  const nextIndexes = [state.gallery.index + 1, state.gallery.index - 1]
    .map((idx) => (idx + state.gallery.urls.length) % state.gallery.urls.length)
    .filter((idx, pos, arr) => arr.indexOf(idx) === pos && idx !== state.gallery.index);
  nextIndexes.forEach((idx) => {
    const img = new Image();
    img.decoding = 'async';
    img.src = getGalleryImageUrl(state.gallery.urls[idx]) || state.gallery.urls[idx];
  });
}
...
function buildCloudinarySrcSet(url, widths = []) {
  const raw = String(url || '').trim();
  if (!raw || !raw.includes('res.cloudinary.com') || !raw.includes('/image/upload/')) return '';
  return widths
    .filter((width) => Number.isFinite(width) && width > 0)
    .map((width) => `${optimizeCloudinaryImage(raw, { width, height: Math.round(width * 1.25), crop: 'fill' })} ${Math.round(width)}w`)
    .join(', ');
}
...
function bindExternalLinkButton(button, getUrl) {
  if (!button) return;
  button.dataset.linkButton = 'true';
  ['click', 'keydown'].forEach((type) => button.addEventListener(type, (event) => {
    if (type === 'keydown' && !['Enter', ' '].includes(event.key)) return;
    if (type === 'keydown') event.preventDefault();
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
    const targetButton = event.target instanceof Element ? event.target.closest('[data-link-button="true"]') : null;
    if (!targetButton || targetButton !== button) return;
