...
const APP_SW_VERSION = 'joodkids-fast-secure-v9';
...
function bindUI() {
  ...
  el.adminDrawer.addEventListener('pointerdown', (event) => { event.stopPropagation(); }, true);
...
function syncGallery() {
  ...
  el.modalImage.src = getGalleryImageUrl(url);
  ...
  state.gallery.urls.forEach((imageUrl, index) => {
    ...
    button.innerHTML = `<img src="${escapeAttr(getMiniImageUrl(imageUrl))}" alt="thumb" loading="lazy" decoding="async" />`;
...
function optimizeCloudinaryImage(url, { width = 720, height = 900, crop = 'fill' } = {}) {
  ...
  const transform = `f_auto,q_auto,dpr_auto,c_${crop},g_auto,w_${Math.round(width)},h_${Math.round(height)}`;
...
function getGalleryImageUrl(url) {
  return optimizeCloudinaryImage(url, { width: 1400, height: 1750, crop: 'limit' }) || url;
}
