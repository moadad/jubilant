...
const state = {
  ...
  cart: loadLocalJSON('joodkids_cart_series', []),
  authUser: null,
  isAdmin: false,
...
function bindUI() {
  ...
  el.searchInput.addEventListener('input', () => { state.filter.search = el.searchInput.value.trim().toLowerCase(); applyFilters(); });
  el.sortFilter.addEventListener('change', () => { state.filter.sort = el.sortFilter.value; applyFilters(); });
...
function initTabs() {
  el.adminTabs.forEach((btn) => btn.addEventListener('click', () => {
...
function subscribeData() {
  onSnapshot(collection(db, 'products'), (snapshot) => {
    state.products = snapshot.docs.map((entry) => ({ id: entry.id, ...entry.data() }));
...
function renderStorefront() {
  ...
  setLink(el.modalFactoryMapLink, state.company.factoryMap);
  setLink(el.modalShopMapLink, state.company.shopMap);
...
function renderMenu() {
  ...
  el.menuCategoryList.appendChild(makeFilterChip('الكل', selectedCat === 'all', () => { state.filter.category = 'all'; applyFilters(); }));
  getCodeCategoryKeys().forEach((key) => {
    el.menuCategoryList.appendChild(makeFilterChip(getCodeCategoryLabel(key), selectedCat === key, () => { state.filter.category = key; applyFilters(); closeDrawers(); }));
  });
  el.menuSeasonList.innerHTML = '';
  el.menuSeasonList.appendChild(makeFilterChip('الكل', selectedSeason === 'all', () => { state.filter.season = 'all'; applyFilters(); }));
  getSeasonOptions().forEach((season) => {
    el.menuSeasonList.appendChild(makeFilterChip(season, selectedSeason === season, () => { state.filter.season = season; applyFilters(); closeDrawers(); }));
  });
...
function applyFilters() {
  ...
  let items = getVisibleProducts().filter((product) => {
    const haystack = `${product.name || ''} ${product.model || ''} ${getCodeCategoryLabel(product.codeCategory)} ${product.season || ''}`.toLowerCase();
      ...
      <div class="product-body">
        ...
        <div class="price-stack">
          <div class="price-main">
            <small>سعر الجملة / السيري</small>
            <strong>${formatCurrency(displayPrice)}</strong>
            ${offer ? `<del>${formatCurrency(toNumber(product.priceWholesale))}</del>` : ''}
          </div>
          <div class="muted">${escapeHTML(getSeriesLabel(product))}</div>
        </div>
...
function renderCart() {
  saveLocalJSON('joodkids_cart_series', state.cart);
  ...
  state.cart.forEach((item) => {
    ...
    card.innerHTML = `
      <img class="cart-thumb" src="${escapeAttr(item.imageUrl || placeholderImage(item.name || 'Jood Kids'))}" alt="${escapeAttr(item.name || '')}" />
      <div>
        <h4>${escapeHTML(item.name || '')}</h4>
        <div class="muted">موديل ${escapeHTML(item.model || '')}</div>
        <div class="muted">${escapeHTML(getSeriesLabel(item))}</div>
        <strong>${formatCurrency(toNumber(item.unitPrice))}</strong>
      </div>
      <div class="qty-box">
...
function renderAdminProducts() {
  ...
  products.forEach((product) => {
    ...
    item.innerHTML = `
      <img class="admin-product-thumb" src="${escapeAttr(urls[0] || placeholderImage(product.name || product.model || 'Jood Kids'))}" alt="${escapeAttr(product.name || '')}" />
      <div>
        ...
        <div class="muted">${hasDiscount(product) ? `خصم ${Math.round(toNumber(product.discountPercent))}% • ` : ''}${formatCurrency(getDisplayPrice(product))} • ${escapeHTML(getSeriesLabel(product))}</div>
      </div>
...
async function saveProduct() {
  ...
  if (!name || !model || Number.isNaN(priceWholesale) || !seriesQtyText) return showToast('الاسم والموديل والسعر وكمية السيري مطلوبة');
  const payload = {
    name,
    model,
    priceWholesale,
...
function populateProductForm(product) {
  ...
  el.productPriceInput.value = String(toNumber(product.priceWholesale));
...
async function addToCart(productId) {
  ...
  else state.cart.push({ id: product.id, name: product.name, model: product.model, unitPrice, originalPrice: toNumber(product.priceWholesale), discountPercent: toNumber(product.discountPercent || 0), imageUrl: normalizeImageUrls(product.imageUrls)[0] || '', qty: 1, seriesQtyText: getSeriesQtyText(product) });
  renderCart();
...
async function submitOrder() {
  ...
  const order = {
    ...
    items: state.cart.map((item) => ({ productId: item.id, name: item.name, model: item.model, unitPrice: item.unitPrice, qty: item.qty, seriesQtyText: getSeriesQtyText(item), imageUrl: item.imageUrl })),
...
async function importProductsExcel(event) {
  ...
  try {
    ...
    rows.forEach((row) => {
      ...
      const priceWholesale = Number(firstValue(row, ['priceWholesale', 'price', 'سعر الجملة']));
      if (!name || !model || Number.isNaN(priceWholesale)) return;
      ...
      batch.set(ref, {
        ...
        seriesQtyText: String(firstValue(row, ['seriesQtyText', 'seriesQty', 'كمية السيري', 'السيري', 'minQty', 'الحد الأدنى']) || '1').trim(),
...
function openDrawer(which) {
  ...
  if (which === 'menu') el.menuDrawer.classList.add('show');
  if (which === 'cart') el.cartDrawer.classList.add('show');
  if (which === 'admin') el.adminDrawer.classList.add('show');
}
...
function openModal(modalId) { id(modalId)?.classList.add('show'); }
...
function syncGallery() {
  const url = state.gallery.urls[state.gallery.index];
  ...
  state.gallery.urls.forEach((imageUrl, index) => {
    ...
    button.innerHTML = `<img src="${escapeAttr(imageUrl)}" alt="thumb" />`;
...
function buildWhatsAppOrderMessage(order) {
  return [
    ...
    'المنتجات:',
    ...(order.items || []).map((item) => `- ${item.name || ''} | موديل ${item.model || ''} | السيري ${getSeriesQtyText(item)} | العدد ${item.qty || 0} سيري | ${formatCurrency(item.unitPrice || 0)}`),
...
function getSeriesLabel(product) {
  ...
  return /^\d+$/.test(text) ? `${text} قطعة في السيري` : text;
}
...
function getDisplayPrice(product) {
  const price = toNumber(product.priceWholesale || 0);
...
function setLink(anchor, url) {
  if (url) { anchor.href = url; anchor.style.pointerEvents = 'auto'; anchor.style.opacity = '1'; }
  else { anchor.href = '#'; anchor.style.pointerEvents = 'none'; anchor.style.opacity = '.45'; }
}
