...
function renderProduct(product, ctx) {
  ...
  const lead = `${title} من ${brand} - ${category} ${product.season ? `• ${product.season}` : ''}`.trim();
  ...
  el.openInStoreBtn.href = `./#${getProductAnchorId(product)}`;
...
function renderBadges(product, category) {
  el.badges.innerHTML = '';
  [category, product.season, product.badgeText, hasDiscount(product) ? `خصم ${Math.round(toNumber(product.discountPercent))}%` : ''].filter(Boolean).forEach((text) => {
  ...
  related.forEach((product) => {
    ...
    article.innerHTML = `
      ...
      <div class="product-body">
        <div class="badges-row"><span class="badge">${escapeHTML(category)}</span></div>
...
function getProductPageUrl(product) {
  ...
  return `${SITE_URL}product.html?id=${encodeURIComponent(String(product?.id || '').trim())}`;
}
