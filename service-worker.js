const APP_VERSION = "joodkids-fast-secure-v11";
...
function isCacheable(request) {
  ...
  const isCloudinaryImage = url.origin.includes('res.cloudinary.com');
  if (!isSameOrigin && !isCloudinaryImage) return false;
    ...
    event.respondWith((async () => {
      ...
      const networkFetch = fetch(request).then(async (response) => {
        if (response && (response.ok || response.type === 'opaque')) {
          cache.put(request, response.clone());
          trimCache(RUNTIME_CACHE, 160).catch(() => {});
        }
        return response;
      }).catch(() => cached);
